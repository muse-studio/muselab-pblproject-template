...
収録する楽曲情報は楽曲名，音楽ジャンル，使用楽器である．Spotifyを主なデータソースとし，スコアや楽曲クレジット情報を元に使用されている楽器構成を抽出した．
一部の楽曲についてスコア（総譜）や詳細な楽曲クレジット情報が不足している場合には，聴取による推定や補足を行った．
ソフトウェア音源を使用して楽器の音色を再現している楽曲については，再現している楽器として抽出した．
また，エフェクトがかかっており元の楽器の音色が判別できない音や，コンピュータで演奏されており楽器の音と判別できない音がある．
これらの楽器は一律に電子音とした．
...
LP-MusicCapsは音声を10秒ごとにテキスト出力する．しかし，10秒では処理時間が長くなるほか，10秒の間に楽器が2度変化する楽曲がいくつか見られた．そこで，分割する秒数を5秒とした．
...
この処理には，音楽音声に対して自然言語による説明文を自動生成するLP-MusicCaps\cite{LP}を用いる．
...
楽器構成の部分一致は考慮せず，全て一致した楽曲のみをカウントすることとする．
これは，どのジャンルにも多く使用されている楽器があるため，楽器構成の部分一致を考慮すると音楽ジャンル一致率の整合性に欠けると判断したためである．
例えば，データベース上でドラムが使用されている楽曲はjazzで50曲中47曲，rockで50曲中50曲，classicで15曲存在し，ヴァイオリンが使用されている楽曲はjazzで50曲中3曲，rockで50曲中1曲，classicで50曲中35曲存在する．
楽器構成の部分一致を考慮した場合のドラムとヴァイオリンが使用されている楽曲を考える．
ドラムが使用されているためどの音楽ジャンルも一致率が比較的高い数値が出てしまうが，実際にヴァイオリンとともに使用されている楽曲はjazzは3曲，rockは1曲，classicは15曲であるため，jazzとrockで高い数値が出るのは整合性に欠けると考えられる．
このように，部分一致は音楽ジャンル一致率の算出における整合性を考慮した結果，楽器構成が完全に一致する楽曲のみをカウントする．
...
算出した一致率を，ヒストグラムで表示する．

ヒストグラムは，横軸を時系列，縦軸を最大値１の音楽ジャンル一致率とする．
セグメントごとに，最も一致率の高いジャンルは赤色に，そのほかのジャンルは青色に表示される．
\figref{fig:exsamhist}はヒストグラム例である．

\begin{figure}[tb]
	\centering
	\includegraphics[width=\hsize]{fig/fig24-examhist.png}
	\caption{出力されるヒストグラムの例}
	\label{fig:exsamhist}
\end{figure}


\section{実装}
本システムはPythonで実装し，以下のライブラリを使用した．
...
対象音声が，初めて解析する音声ファイルである場合，
\verb|Split-audio-to-named-dir|関数を用いて音声ファイルを５秒ごとのセグメントに分割する．
分割されたセグメントを，webサイトを介して処理する．
\verb|Split-audio-to-named-dir|関数のフローチャートを\figref{fig:audiobunkatu}に示す．

指定パスが存在し，ディレクトリであるかを確認後，指定フォルダ内のテキストファイルを処理する．
処理内容は\verb|process-existing-folder|関数の内容である．
\verb|process-existing-folder|関数のフローチャートを\figref{fig:folder}に示す．

\begin{figure}[tb]
	\centering
	\includegraphics[width=\hsize]{fig/fig20-audiobunkatu.png}
	\caption{Split-audio-to-named-dir関数のフローチャート}
	\label{fig:audiobunkatu}
\end{figure}

\begin{figure}[tb]
	\centering
	\includegraphics[width=\hsize]{fig/fig21-folder.png}
	\caption{process-ecisting-folder関数のフローチャート}
	\label{fig:folder}
\end{figure}

%フォルダが存在しない場合，つまり解析したことのないファイルを解析する場合には以下の処理に分岐する．

\subsection{セグメントに含まれる楽器名の抽出}
...
% -- 3章から移動
webサイトを介する処理には，ChromeDriveを用いてwebスクレイピングを行う．ChromeDriverの実行ファイルのパスを指定し，LP-MusicCapsのWebアプリのURLを指定することで，webサイトを介して音声ファイルを送信し，そこで出力されたテキストを取得している．

取得したテキストはセグメントが保存されているフォルダ内にテキストファイルとして保存される．
このとき，外部webサイトを介した処理を行っているため，処理速度が遅くなるという問題がある．
10秒の音声ファイルを処理する場合，2分程の処理時間を要した．通常楽曲は5分前後であることが多く，楽曲を入力した際に長い処理時間を要する．
解決方法として，すでに入力音声ファイルと同名のフォルダが存在する場合にはLP-MusicCapsを介した処理を行わず，フォルダ内に存在するテキストファイルからテキストを取得する処理を追加した．
これにより，過去に解析したことのある楽曲を再度解析する際の処理速度が短縮される．
% ------------
...
引数は”folder-path”（処理対象のフォルダパス），”csv-file-path”（データベースのパス）である．
...
”calculate-genre-match-rate”関数は，楽器名リストとデータフレームを入力として受け取り，音楽ジャンルごとの一致率を計算する関数である．
関数のフローチャートを\figref{fig:rate}に示す．

\begin{figure}[tb]
	\centering
	\includegraphics[width=\hsize]{fig/fig22-rate.png}
	\caption{Calculate-genre-match-rate関数のフローチャート}
	\label{fig:rate}
\end{figure}
...
”plot-genre-histograms”関数は，音楽ジャンルごとのセグメントごとの一致率をヒストグラムで表示する関数である．

関数のフローチャートを\figref{fig:histgram}に示す．

\begin{figure}[tb]
	\centering
	\includegraphics[width=\hsize]{fig/fig23-histgram.png}
	\caption{Plot-genre-histograms関数のフローチャート}
	\label{fig:histgram}
\end{figure}
